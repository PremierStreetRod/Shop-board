...
## 2026-08-06 — block 69: Coyote → board mapper — dry-run preview (v69, no migration)

Owner-rep asked what mapping we can do while still waiting on the developer to confirm the push sends only changed records (vs today's full-record-every-day), noting the push DATA isn't changing — only how/when he pushes. That's right: the intake reducer is latest-per-order and the write step is idempotent keyed on order #, so cadence never changes the mapping (why block 64 was built cadence-agnostic). Also answered the storage worry with measured numbers: avg payload ~3 KB/order, whole intake table ~1 MB today, even full-record-daily worst case ~160 MB/yr (~1.6 GB/decade) — storage won't be the deciding factor. Chose the dry-run preview. New admin-only /mapper page (classifyOrder + mapperPreviewData + mapperPreviewPage; gate mirrors /intake). For every fresh order it shows exactly what the mapping job WOULD create — order # / customer / cab part(s) -> family -> routed line / proposed board state / target ship / invoice-note -> manager-review flag / multi-cab dotted split — grouped: will place (Queued+recognized part), need attention (Hold or unknown part), shipped/skip (Processed = history), excluded (Blazer tops). Writes nothing; cab # is deliberately not invented (the floor/whiteboard owns the counter until cutover, match by order #). Grounded in the real payload (introspected live in Supabase): order.order_number/status/date/ship_date, the invoice note at order.invoice_note, items with item_number + build_it_options. v69 = 421,151 / 1115575037 (commit f958ad0; 3 hash-gated find/repl edits from v68, diff-derived + reversal-proven byte-exact both directions before inject, dual-hash gate; node --check clean). 26/26 unit tests (real functions extracted + mock db: classifyOrder branches, multi-cab split, invoice-note extract, latest-per-order dedup, cab+top->place, group sort/counts). Live E2E fully verified over an active admin session (read-only GET): /mapper rendered real data — 176 raw -> 98 dupes collapsed -> 78 distinct, 20 will-place, 58 shipped-skip, 13 note->review, the demo order split into 6 dotted sub-orders each routed to its line; /health green; bogus path 404s. Next: the write step (idempotent promote order -> board cab, human-in-the-loop) — buildable now, cadence-independent.

## 2026-08-06 — block 68: Pay Worksheet — payroll hours export (v68, no migration)

Replaces the manual "Employee Pay Worksheet" that staff hand-tally and email to the outsourced payroll company. Shop Board already holds accurate clock hours, so it now builds that sheet itself. New admin-only /payroll page + /payroll.csv download (payPeriod / payrollData / payrollPage / payrollCsv). Gate is stricter than Reports (admin-only, not manager+admin, plus a must_change_pin redirect) because payroll is sensitive. Per owner-rep's answers (2026-08-06): HOURS ONLY (no wage rates or pay stored in the app); Regular (up to 8/day) vs Overtime (over 8/day) split; Sick from approved time-off reason "Sick"; Unpaid from reason "Unpaid"; rounded to the nearest quarter-hour; semi-monthly, paid the 1st and 15th (cutoffs 11th-25th paid the 1st, 26th-10th paid the 15th; Custom from-to range overrides). Worked hours come from real clock_event intervals (workIntervals -> overlapHrs, dayed by interval start, clipped to the window); Sick/Unpaid from approved time_off_request by reason (8h per work day). Page shows a period picker (Current/Last/Custom), a per-period totals table, and a day-by-day grid (work days only, OT/S/U cell marks); CSV carries a summary block plus day-by-day detail. v68 = 408,681 / 3709161518 (commit 3f0b5e2; 3 hash-gated find/repl edits from v67, reversal-proven byte-exact before inject, dual-hash gate on dispatch; node --check clean). 17/17 unit tests (reg/ot split at 8/day, quarter-rounding, sick/unpaid per-work-day counting, semi-monthly boundaries incl. year rollover and last-period). Payroll nav link added between Reports and Integrity. Follow-up: add "Unpaid" to the time_off_reason pick list (not seeded yet, so the Unpaid column reads 0 until added). Live: /health green; /payroll + /payroll.csv deployed and admin-gated (302 to /login like /reports, while a bogus path 404s -> routes exist in shipped v68); rendered page not click-tested (admin session expired, sign-in needs a PIN the harness won't type).
