...
http.createServer(async (req, res) => {
  ...
  // Q123: Content-Security-Policy — shipped REPORT-ONLY first (it reports what
  // WOULD be blocked to the browser console but blocks nothing), so it can be
  // proven clean on every real page before we enforce. Inventory (block 45)
  ...
  res.setHeader("Content-Security-Policy-Report-Only", "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; font-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'; frame-ancestors 'none'; worker-src 'self'");
